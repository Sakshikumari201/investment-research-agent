  ...
  return (
    <div className="flex-1 flex flex-col min-h-screen overflow-y-auto px-8 py-6 relative">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 border-b border-[#0e1627] pb-5 gap-4 relative z-10 font-tech">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-[10px] font-bold tracking-widest uppercase flex items-center gap-1.5 border transition-all duration-300 ${
              isInterviewMode
      ...
      >
        ...
        <div className="flex items-center gap-3 bg-slate-950/60 px-4 py-1.5 rounded-xl border border-slate-900">
          <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest font-mono">PERSONA_SYS:</label>
          <select
            value={persona}
            ...
            disabled={loading}
          >
            <option value="GROWTH" className="bg-slate-950 text-slate-300">Growth Aggregator</option>
            <option value="VALUE" className="bg-slate-950 text-slate-300">Value Audit</option>
            <option value="DIVIDEND" className="bg-slate-950 text-slate-300">Dividend Yield Yield</option>
      ...
      {loading && (
        <div className="hud-panel rounded-2xl p-8 mb-8 flex flex-col md:flex-row gap-8 items-center justify-between border-accent-500/20 relative overflow-hidden z-10">
          ...
          <div className="flex-1 flex flex-col gap-4 font-tech w-full">
            ...
            <div className="bg-slate-950/80 border border-slate-900 p-4 rounded-xl font-mono text-[10px] text-accent-400/90 leading-relaxed shadow-inner">
              <span className="text-slate-600 block">// STREAMING LIVE PIPELINE DIAGNOSTICS...</span>
              <span className="text-slate-400 block mt-1">
                &gt; SYSTEM_TICK: {Date.now()}<br />
                &gt; NODE_STATUS: RUNNING<br />
                &gt; THREAD_PROCESSOR: OK
              </span>
              <p className="text-emerald-400 font-bold mt-2 border-t border-slate-900 pt-2 flex items-center gap-1.5">
                <Terminal size={12} />
      ...
      {result && !loading && (
        <div className="flex flex-col gap-8 relative z-10">
          ...
          {result.isCached && (
            <div className="hud-panel hud-panel-amber rounded-xl px-4.5 py-3 flex flex-wrap justify-between items-center text-[10px] font-mono font-bold tracking-wider gap-2">
              <span className="text-amber-400 flex items-center gap-1.5 animate-pulse">
                ⚡ SYSTEM_CACHE_HIT: Served from cache (TTL: 10m) to optimize LLM credits.
              </span>
              <span className="text-slate-500">TIMESTAMP: {result.cachedTimeAgo || 'just now'}</span>
            </div>
          )}
          ...
          <div className="hud-panel rounded-2xl p-6 md:p-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-8 relative overflow-hidden">
            ...
            <div className="flex-1 flex flex-col gap-3.5 z-10 font-tech">
              <div className="flex items-center gap-3">
                ...
                <span className="text-[10px] font-bold font-mono text-accent-400 bg-accent-950 border border-accent-900/60 px-2 py-0.5 rounded tracking-widest">{result.ticker}</span>
                <button
              ...
              <div className="flex flex-wrap items-center gap-8 mt-2">
                <div className="flex flex-col">
                  <span className="text-[8px] uppercase font-bold text-slate-500 tracking-widest font-mono">DECISION_RECOMMENDATION</span>
                ...
                <div className="flex flex-col border-l border-slate-900 pl-8">
                  <span className="text-[8px] uppercase font-bold text-slate-500 tracking-widest font-mono">STOCK_UNIT_VAL</span>
                ...
                <div className="flex flex-col border-l border-slate-900 pl-8">
                  <span className="text-[8px] uppercase font-bold text-slate-500 tracking-widest font-mono">AUDIT_STYLE</span>
                  <span className="text-[9px] font-bold bg-indigo-950/40 text-indigo-400 border border-indigo-900/60 px-2.5 py-0.5 rounded-full mt-1.5 tracking-wider">
              ...
              <div className="flex flex-col gap-3 font-mono">
                ...
                <div className="pl-5 flex flex-col gap-2.5 border-l border-slate-900/60 mt-1">
                  <div className="flex justify-between items-center text-[10px]">
                    <span className="text-slate-500">├─ FINANCIAL_STRENGTH</span>
                    <span className="font-bold text-slate-300">{result.decision.scoreBreakdown.financialStrength} / 25</span>
                  </div>
                  <div className="flex justify-between items-center text-[10px]">
                    <span className="text-slate-500">├─ MARKET_SENTIMENT</span>
                    <span className="font-bold text-slate-300">{result.decision.scoreBreakdown.marketSentiment} / 20</span>
                  </div>
                  <div className="flex justify-between items-center text-[10px]">
                    <span className="text-slate-500">├─ COMPETITIVE_MOAT</span>
                    <span className="font-bold text-slate-300">{result.decision.scoreBreakdown.competitiveMoat} / 20</span>
                  </div>
                  <div className="flex justify-between items-center text-[10px]">
                    <span className="text-slate-500">├─ RISK_RESILIENCE</span>
                    <span className="font-bold text-slate-300">{result.decision.scoreBreakdown.risksResilience} / 20</span>
                  </div>
                  <div className="flex justify-between items-center text-[10px]">
                    <span className="text-slate-500">└─ NEWS_SENTIMENT_WEIGHT</span>
                    <span className="font-bold text-slate-300">{result.decision.scoreBreakdown.newsSentiment} / 15</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="hud-panel rounded-2xl p-6">
              ...
              <div className="flex flex-col gap-2.5 font-mono">
                {result.decision.confidenceFactors.map((item, idx) => (
                  <div key={idx} className="flex items-center justify-between text-[10px] p-2.5 bg-slate-950/40 rounded-lg border border-slate-900">
                    <span className="text-slate-400 truncate max-w-[170px]">{item.factor}</span>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="hud-panel rounded-2xl p-6 lg:col-span-2 relative">
              ...
              <div className="mb-6 font-mono text-[9px]">
                <div className="flex justify-between text-slate-500 mb-1.5 uppercase font-bold tracking-widest">
                ...
                <div className="h-1.5 w-full bg-slate-950 rounded-full overflow-hidden p-0.5 border border-slate-900 flex">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="hud-panel hud-panel-emerald rounded-xl p-4 bg-emerald-950/5 flex flex-col">
                  <div className="flex items-center gap-2">
                    <div className="w-6 h-6 rounded-full bg-emerald-950 flex items-center justify-center font-bold text-emerald-400 text-[10px] border border-emerald-900 font-mono shadow-[0_0_8px_rgba(16,185,129,0.3)]">
                      B
                    </div>
                    <span className="text-[10px] font-black text-emerald-400 uppercase tracking-widest font-tech">
                      BULL_THESIS_OPTIMIZATION
                    </span>
                  </div>
                  <div className="text-[10px] text-slate-300 leading-relaxed font-mono whitespace-pre-line mt-3 bg-slate-950/40 p-3 rounded-lg border border-slate-900/60 overflow-y-auto max-h-52">
                    {result.bullAnalysis}
                  </div>
                </div>
                ...
                <div className="hud-panel hud-panel-rose rounded-xl p-4 bg-rose-950/5 flex flex-col">
                  <div className="flex items-center gap-2">
                    <div className="w-6 h-6 rounded-full bg-rose-950 flex items-center justify-center font-bold text-rose-400 text-[10px] border border-rose-900 font-mono shadow-[0_0_8px_rgba(239,68,68,0.3)]">
                      A
                    </div>
                    <span className="text-[10px] font-black text-rose-400 uppercase tracking-widest font-tech">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="hud-panel rounded-2xl p-6 relative">
              ...
              <p className="text-[10px] text-slate-500 mb-4 font-tech uppercase tracking-wider">
              ...
              <div className="flex flex-col sm:flex-row items-center gap-8 font-tech">
                ...
                <div className="w-40 h-40 rounded-full border border-slate-900 flex items-center justify-center relative shrink-0">
                  ...
                  <div className="w-28 h-28 rounded-full bg-[#050811] flex flex-col items-center justify-center border border-slate-900 text-center relative z-10 shadow-[0_0_10px_rgba(0,0,0,0.8)]">
                    <span className="text-[8px] text-slate-500 font-bold uppercase tracking-widest">Target allocation</span>
                    <span className="text-base font-black text-accent-400 font-mono mt-0.5">{result.decision.portfolioSimulation.allocation[0]?.percentage || 0}%</span>
                  </div>
                </div>

                <div className="flex-1 flex flex-col gap-2.5 w-full font-mono text-[10px]">
              ...
              <p className="text-[10px] text-slate-400 mt-5 italic bg-slate-950/60 p-3 rounded-lg border border-slate-900 leading-relaxed font-mono">
                &gt; "{result.decision.portfolioSimulation.explanation}"
              </p>
            </div>
              ...
              <div className="flex flex-col gap-3 max-h-60 overflow-y-auto pr-1 scrollbar-thin">
                {result.decision.newsTimeline && result.decision.newsTimeline.length > 0 ? (
                  result.decision.newsTimeline.map((item, idx) => (
                    <div key={idx} className="flex gap-4 items-start text-[10px] border-b border-slate-900/60 pb-3 last:border-b-0 font-mono">
                      <div className="shrink-0 flex flex-col items-center">
                        <span className="text-[8px] text-slate-500 font-bold uppercase">{item.date}</span>
                        <span className={`px-1.5 py-0.5 rounded text-[7px] font-black uppercase mt-1 ${
                  ...
                  <p className="text-[10px] text-slate-500 italic font-mono">// No news timeline events logged.</p>
                )}
              </div>
            </div>
          </div>
          ...
          {isInterviewMode && (
            <div className="hud-panel rounded-2xl p-6 border-indigo-500/20 bg-indigo-950/5 animate-slide-down font-tech">
              ...
              <p className="text-[10px] text-slate-500 mb-5 uppercase tracking-wide">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5 font-mono text-[10px]">
                <div className="bg-[#03060c] border border-slate-900 p-4 rounded-xl flex flex-col gap-2">
                  <h4 className="font-bold text-white uppercase tracking-wider text-slate-300 border-b border-slate-900 pb-1.5 text-[9px]">// QUESTION 01: Why justify recommendation despite high valuation multiples?</h4>
                ...
                <div className="bg-[#03060c] border border-slate-900 p-4 rounded-xl flex flex-col gap-2">
                  <h4 className="font-bold text-white uppercase tracking-wider text-slate-300 border-b border-slate-900 pb-1.5 text-[9px]">// QUESTION 02: How does the pipeline discount sentiment volatility?</h4>
                  <p className="text-slate-400 leading-relaxed text-[9px]">
